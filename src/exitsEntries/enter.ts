...
const trendFollowingEntry = (
  ...
  if (
    indicator.rsi.currentRsi > config.rsiThreshold &&
    indicator.rsi.currentRsi < 70 &&
    indicator.rsi.previousRsi < config.rsiThreshold
  ) {
    ...
    if (
      indicator.macd.macd > indicator.macd.signal && // MACD crossover
  ...
  if (indicator.rsi.currentRsi > config.rsiThreshold) {
    if (indicator.macd.macd > indicator.macd.signal && indicator.macd.macd > 0) {
      ...
      if (indicator.volumeSpike) {
        ...
        if (atrPercent > 0.1 && atrPercent < 1.0) {
